...
class _DashboardViewState extends State<DashboardView> {
  ...
  Future<void> _scanCrop() async {
    try {
      ...
      var result = await _aiService.analyzeImage(photo.path, _selectedCrop, lat: lat, lng: lng);
  ...
  // ... inside _DashboardViewState

  bool _isGeneratingReport = false;
  Map<String, dynamic>? _cloudReport;
  ...
  void _handleResult(Map<String, dynamic> result) {
      setState(() {
        _isScanning = false;
        _lastResult = result;
        _cloudReport = null; // Reset previous report
      ...
      if (result['confidence'] > 0.8) {
         ...
         // --- HYBRID FLOW: Automatic Cloud Check ---
         if (result['label'] != 'Healthy') {
            _checkAndGenerateCloudReport(result['image_path'], result['label']);
         }
  ...
  Future<void> _checkAndGenerateCloudReport(String imagePath, String label) async {
    // 1. Check Internet silently
    bool hasInternet = await _aiService.hasInternet(); // We need to make this public or duplicate logic
    // actually _hasInternet is private in AIService. Let's assume we can call generateCloudReport and it handles the check.
    
    setState(() => _isGeneratingReport = true);
    
    // Get Lat/Lng from local result if available
    double? lat = _lastResult?['latitude'];
    double? lng = _lastResult?['longitude'];

    // 2. Call Cloud
    var report = await _aiService.generateCloudReport(imagePath, label, _selectedCrop, lat: lat, lng: lng);
    
    if (mounted) {
      setState(() {
        _isGeneratingReport = false;
        if (report.containsKey('error')) {
          print("Cloud Report Failed: ${report['error']}");
        } else {
          _cloudReport = report;
          // Auto-speak the summary if available
          if (_cloudReport!['audio_summary'] != null) {
             ScaffoldMessenger.of(context).showSnackBar(SnackBar(
               content: Text("AI Expert Analysis Received!"),
               backgroundColor: Colors.purple,
               duration: Duration(seconds: 4),
             ));
             // Delay slightly so it doesn't overlap with local TTS
             Future.delayed(Duration(seconds: 5), () {
                _speak(_cloudReport!['audio_summary']);
             });
          }
        }
      });
      ...
      children: [
        ...
        if (_isGeneratingReport)
           Padding(
             ...
             child: LinearProgressIndicator(backgroundColor: Colors.purple[100], color: Colors.purple),
           ),
        
        if (_cloudReport != null)
           Card(
             color: Colors.purple[50], // Distinct color for "Premium/Cloud" feature
                 ...
                 children: [
                   Row(
                     children: [
                       ...
                       SizedBox(width: 10),
                       Text(_cloudReport!['title'] ?? "AI Expert Analysis", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.purple[900])),
                     ],
                   ),
                   SizedBox(height: 10),
                   Text(_cloudReport!['ai_insight'] ?? "", style: TextStyle(fontStyle: FontStyle.italic, color: Colors.black87)),
                   SizedBox(height: 10),
                   Divider(color: Colors.purple[200]),
                   Text("Personalized Plan:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.purple[900])),
                   if (_cloudReport!['personalized_plan'] is List)
                      ...(_cloudReport!['personalized_plan'] as List).map((p) => Padding(
